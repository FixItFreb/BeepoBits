...
func decompress_lzw(code_stream_data: PackedByteArray, min_code_size: int, colors: PackedByteArray) -> PackedByteArray:
	...
	# 🍲 : The data may not start with a clear code. In such
	# cases, removing the first code will break stuff
	
	# Remove first Clear Code from stream. We don't need it.
#	var _temp = binary_code_stream.read_bits(current_code_size)
	
	# 🍲 : Instead, read the first code normally, and check if it is,
	# in fact, the clear code
	
	# let CODE be the first code in the code stream
	var code: int = binary_code_stream.read_bits(current_code_size)
	var prevcode: int = 0
	
	# 🍲 : If it is, We discard it by simply reading another code from the stream
	...
	index_stream.append_array(code_table.gets(code).sequence)
	prevcode = code
...
#			break
		...
		var code_entry: CodeEntry = code_table.gets(code)
		if code_entry != null:  # if YES
			...
			code_table.add(code_table.gets(prevcode).add(k))
			# set PREVCODE = CODE
			prevcode = code
		else:  # if NO
			# let k be the first index of {PREVCODE}
			var prevcode_entry: CodeEntry = code_table.gets(prevcode)

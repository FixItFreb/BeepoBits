...
namespace uOSC
{
    ...
    public class Parser
    {
        ...
        public void Parse(byte[] buf, ref int pos, int endPos, ulong timestamp = 0x1u)
        {
            ...
            if (pos != endPos)
            {
                GD.PrintErr(System.String.Format("The parsed data size is inconsitent with the given size: {0} / {1}", pos, endPos));
            }
        }
        ...
        void ParseBundle(byte[] buf, ref int pos, int endPos)
        {
            ...
            while (pos < endPos)
            {
                ...
                {
                    GD.PrintErr(System.String.Format("Given data is invalid (bundle size ({0}) is not a multiple of 4).", contentSize));
                    pos += contentSize;
                }
            }
        }
